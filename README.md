...
- Login multi-role (`dev`, `staff`, `viewer`) + logs aktivitas input
...
- Akun pertama yang register akan otomatis menjadi role `dev`.

## Role Akses

- `dev`: akses penuh (tambah/edit/hapus, import excel, melihat logs input, ubah role user)
- `staff`: tambah/edit/hapus data project, biaya, absensi (tanpa import excel)
...
- `/logs` : Khusus role `dev`, berisi logs input dan manajemen role user.
