          ...
          <table className="w-full table-fixed text-[11px] leading-5 sm:text-xs">
            <thead>
              <tr className="text-left text-slate-500">
                ...
                <th className="pb-2 font-medium">User & Aktivitas</th>
              ...
              {filteredLogs.map((log) => {
                ...
                return (
                  <tr key={log.id} className="border-t border-slate-100 align-top">
                    <td className="py-2 pr-2 text-slate-600">{formatDateTime(log.createdAt)}</td>
                    <td className="py-2 pr-2">
                    ...
                    <td className="py-2 pr-2 text-slate-700">{log.description}</td>
                    <td className="py-2 pr-2">
          ...
          <section className="modal-card panel relative z-10 max-h-[calc(100vh-2rem)] w-full max-w-4xl overflow-y-auto p-5">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold text-slate-500">Waktu</p>
                <p className="mt-1 text-xs text-slate-800">{formatDateTime(selectedLog.createdAt)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold text-slate-500">User</p>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold text-slate-500">Aksi / Modul</p>
                <p className="mt-1 text-xs text-slate-800">
                  {selectedLog.actionType} / {selectedLog.module}
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold text-slate-500">Entitas</p>
                <p className="mt-1 text-xs text-slate-800">{selectedLog.entityName ?? "-"}</p>
                <p className="font-mono text-[11px] text-slate-500 break-all">
                  {selectedLog.entityId ?? "-"}
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 sm:col-span-2">
                <p className="text-[11px] font-semibold text-slate-500">ID Log</p>
                <p className="mt-1 font-mono text-[11px] text-slate-700 break-all">{selectedLog.id}</p>
              </div>
            </div>
