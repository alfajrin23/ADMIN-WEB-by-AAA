...
export async function createAttendanceAction(formData: FormData) {
  ...
  if (activeDataSource === "excel") {
    ...
    if (!ensureSupabaseWriteConfigured(returnTo, "Gagal menyimpan data absensi.")) {
      return;
    }
    ...
    if (result.error) {
      redirect(withReturnMessage(returnTo, "error", getSupabaseMutationErrorMessage("Gagal menyimpan data absensi.")));
    }
...
export async function updateAttendanceAction(formData: FormData) {
  ...
  if (activeDataSource === "excel") {
    ...
    if (!ensureSupabaseWriteConfigured(returnTo, "Gagal memperbarui data absensi.")) {
      return;
    }
    const draftProjectId = payload.project_id || await ensureSupabaseAttendanceDraftProjectId(supabase);
    if (!draftProjectId) {
    ...
    if (result.error) {
      redirect(withReturnMessage(returnTo, "error", getSupabaseMutationErrorMessage("Gagal memperbarui data absensi.")));
    }
  } else if (activeDataSource === "firebase") {
    const firestore = getFirestoreServerClient();
    if (!firestore) {
...
export async function prepareAttendanceExportAction(formData: FormData) {
  ...
  if (activeDataSource === "excel") {
    ...
    if (!ensureSupabaseWriteConfigured(returnTo, "Gagal menyimpan finalisasi rekap.")) {
      return;
    }
    ...
    if (attendanceResult.error || payrollResult.error) {
      redirect(withReturnMessage(returnTo, "error", getSupabaseMutationErrorMessage("Gagal menyimpan finalisasi rekap.")));
    }
  } else if (activeDataSource === "firebase") {
    const firestore = getFirestoreServerClient();
    if (!firestore) {
...
export async function deleteAttendanceAction(formData: FormData) {
  ...
  if (activeDataSource === "excel") {
    ...
    if (!ensureSupabaseWriteConfigured(returnTo, "Gagal menghapus data absensi.", formData)) {
      return optimisticActionError(getSupabaseMutationErrorMessage("Gagal menghapus data absensi."));
    }
    ...
    if (error) {
      return returnOptimisticErrorOrRedirect(
        formData,
        returnTo ?? "/attendance",
        getSupabaseMutationErrorMessage("Gagal menghapus data absensi."),
      );
    }
  } else if (activeDataSource === "firebase") {
    const firestore = getFirestoreServerClient();
  ...
  if (isOptimisticUiRequest(formData)) {
    return optimisticActionSuccess("Data absensi berhasil dihapus.");
  }
  if (returnTo) {
