...
export async function autoChecklistKmpMaterialsForCompletedProjects(projectIds: string[]) {
  const targetProjectIds = Array.from(new Set(projectIds.map((projectId) => projectId.trim()).filter(Boolean)));
  if (targetProjectIds.length === 0 || activeDataSource === "demo") {
    return { projectIds: [], rowCount: 0 };
  }

  revalidateExpenseCache();
  const { getKmpCianjurMaterialRequesterName, getKmpCianjurMissingMaterialReport } = await import("@/lib/data");
  const [report, requesterName] = await Promise.all([
    getKmpCianjurMissingMaterialReport(),
    getKmpCianjurMaterialRequesterName(),
  ]);
  const targetSet = new Set(targetProjectIds);
  const expenseDate = getCurrentJakartaDate();
  const rows = report.projects
    .filter((project) => targetSet.has(project.projectId))
    .flatMap((project) =>
      project.missingMaterialDetails.map((material) => ({
        id: createExpenseMutationId({
          mode: "kmp_material_check",
          submissionToken: "project-status-selesai",
        ...
        requester_name: material.submissionName || requesterName,
        description: material.materialName || material.materialLabel,
        recipient_name: null,
        quantity: 1,
        unit_label: null,
        usage_info: `Checklist Material KMP Cianjur - ${material.materialLabel} - tanpa nominal`,
        unit_price: 0,
        amount: 0,
  ...
  if (activeDataSource === "excel") {
    ...
    for (const chunk of chunkArray(rows, 100)) {
      const { error } = await supabase.from("project_expenses").upsert(
        chunk.map(({ project_name: _projectName, ...row }) => row),
        { onConflict: "id" },
      );
      if (error) {
        console.warn("[kmp-material] gagal auto checklist saat project selesai.", error.message);
        return { projectIds: [], rowCount: 0 };
      }
    }
  ...
  const checkedProjectIds = Array.from(new Set(rows.map((row) => row.project_id)));
  revalidateExpenseCache();
  return { projectIds: checkedProjectIds, rowCount: rows.length };
}
