...
import { buildExpenseRecapClipboardText } from "@/lib/expense-clipboard";
...
export async function GET(request: Request) {
  ...
  if (selectedOnly && requestedProjectIds.length === 0) {
    return new Response("Pilih minimal satu project untuk salin rekapan.", { status: 400 });
  }
  ...
  const text = buildExpenseRecapClipboardText(
